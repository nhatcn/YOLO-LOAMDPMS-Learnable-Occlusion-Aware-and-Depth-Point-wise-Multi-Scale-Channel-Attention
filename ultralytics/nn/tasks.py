...
class BaseModel(torch.nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        if isinstance(x, dict):  # for training case
    ...
    def _predict_augment(self, x):
        LOGGER.warning(f"{self.__class__.__name__} does not support 'augment=True' prediction. Reverting to single-scale prediction.")
        return self._predict_once(x)

    def info(self, verbose=True, imgsz=640):
        """
        Prints model summary and information.

        Args:
            verbose (bool): If True, prints detailed information about the model.
            imgsz (int): Input image size for model summary.
    ...
    def is_fused(self):
        ...
        return not any(isinstance(m, (Conv, DWConv)) and hasattr(m, 'bn') for m in self.modules())

    def _apply(self, fn):
        """Apply function to all tensors in the model."""
        self = super()._apply(fn)
        if hasattr(self, 'model') and isinstance(self.model, nn.ModuleList):
            for m in self.model:
                m._apply(fn)
        return self
    ...
    def load(self, weights, verbose=True):
        ...
        state_dict = intersect_dicts(state_dict, self.state_dict())
        self.load_state_dict(state_dict, strict=False)
        if verbose:
            LOGGER.info(f'Transferred {len(state_dict)}/{len(self.state_dict())} items from pretrained weights')
    ...
    def init_criterion(self):
        """Initialize the loss criterion (to be implemented by subclasses)."""
        raise NotImplementedError("init_criterion must be implemented in subclass")
